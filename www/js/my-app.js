      ...
      $.each(data, function(index, value) {
        $$('#content-block-main').append('<div class="card ks-facebook-card">' +
          '<div class="card-header">' +
              '<div id="catIcon" class="ks-facebook-avatar"><i class="f7-icons">bookmark</i></div>' +
            ...
            '<a  href="#" id="post-bookmark-modal" data-bookmark="'+value.id+'" class="button item-link external bookmark-modal post-bookmark-modal"><i class="f7-icons padtop">bookmark</i></a>'+
            '<a  href="#" data-popup=".popup-doc" data-post="'+value.id+'" id="view-post-btn" class="button open-popup view-post-btn">View</a></div>' +
          ...
          '<div class="card-header">' +
            ...
            '<div class="card-content">' + 
              ...
              '<p><h3>'+data.title.rendered+'</h3></p>' +
               ...
               '<p class="buttons-row"><a href="'+data.link+'" class="button button-raised button-fill color-teal item-link external" style="font-weight:bold;">Read More</a><br/><br/><a  href="whatsapp://send?text='+data.link+'" style="text-align:center;font-weight:bold;" class="button button-raised button-fill color-teal item-link external">Share Post</a></p>'+
  ...
  success: function(data){
        ...
        if(updateVersion > currentVersion){
           myApp.notification.create({
            ...
            closeIcon: true,
            closeOnClick: true,
            message: '<a href="http://spaneapp.com/spaneapp.apk" class="link external">Click here to <b>Download</b> new App version</a>Your network seems to have a problem loading new content',
            closeButton: true
            }).open();
     }
      ...
      $.each(offlineData, function(index, value) {
            $$('#content-block-main').append('<div class="card ks-facebook-card">' +
              ...
              '<div class="ks-facebook-date">'+value.date+'</div>' +
                ...
                '<a  href="whatsapp://send?text='+value.link+'" class="button item-link external"><img src="img/whatsapp_share.png" height="20px" style="margin-top:8px;"></a>'+
                '<a  href="#" id="post-bookmark-modal" data-bookmark="'+value.id+'" class="button item-link external bookmark-modal post-bookmark-modal"><i class="f7-icons padtop">bookmark</i></a>'+
                '<a  href="posts.html?postid='+value.id+'" data-popup=".popup-doc" data-post="'+value.id+'" id="view-post-btn" class="button open-popup view-post-btn">View</a></div>' +
              '<div class="item-inner"><div class="item-title"></div>'+
        '</div>');
        fDate();
        $('#loader-image').hide();
...
function postBookMark(){
   ...
   $$('.post-bookmark-modal').on('click', function () {
              ...
              success: function(data){
               ...
               alert('Post bookmarked, check Bookmarks to see your Post');
                myApp.notification.create({
                  ...
                  closeIcon: true,
                  closeOnClick: true,
                  message: 'Post bookmarked, check Bookmarks to see your Post',
                  closeButton: true
                  }).open();
              },
              complete: function(){
...
function removeBookmarks(offlineData, value){
    $('#bookmark-modal').click(function(){
      ...
      var indexData = $$(this).attr('data-index');
              ...
              closeIcon: true,
              closeOnClick: true,
              message: 'Your Post has been Removed in Bookmarks',
              closeButton: true
            }).open();
